...
#
# Saída: prints no console + CSV resumo em Backend/ml/houseOdds/house_odds_summary.csv
# ------------------------------------------------------------
...
def pick_odds_1x2(row: pd.Series) -> Tuple[Optional[float], Optional[float], Optional[float]]:
    ...
    (mesma lógica do seu build_dataset.py)
    """
...
def main():
    ...
    # Paths
    # -------------------------
    THIS_DIR = os.path.dirname(os.path.abspath(__file__))            # .../Backend/ml/houseOdds
    ...
    for fp in sorted(files):
        ...
        # precisa de colunas mínimas
        if not {"Home", "Away", "Date"}.issubset(set(df.columns)):
            continue

        # FullTime pode não existir
        if "FullTime" not in df.columns:
            df["FullTime"] = None

        # garante colunas de odds (se não existirem, row.get vai virar None)
        needed_odds_cols = [
        ...
        for _, r in df.iterrows():
            ...
            # casa "aposta" no maior prob (menor odd) entre H/D/A
    ...
    def summarize(s: Stats) -> Dict[str, float]:
        ...
        if s.n_rows_with_odds > 0:
            # médias raw por classe (prob implícita = 1/odd)
    ...
    def fmt_prob_from_odd(o: float) -> str:
        if o != o or o <= 0:
            return "NaN"
        return f"{(1.0/o)*100:.2f}%"
    ...
    ohm = total.get("mean_odd_home", float("nan"))
    odm = total.get("mean_odd_draw", float("nan"))
    oam = total.get("mean_odd_away", float("nan"))
    oum = total.get("mean_odd_universe", float("nan"))
    ofav = total.get("mean_odd_favorite", float("nan"))

    print("\nOdds médias selecionadas (pré-jogo) POR RESULTADO:")
    print(f" - Mandante: {ohm:.4f}  -> prob implícita ~ {fmt_prob_from_odd(ohm)}")
    print(f" - Empate:   {odm:.4f}  -> prob implícita ~ {fmt_prob_from_odd(odm)}")
    print(f" - Visitante:{oam:.4f}  -> prob implícita ~ {fmt_prob_from_odd(oam)}")

    print("\nOdds médias (UNIVERSO):")
    print(f" - Universo (todas odds H/D/A juntas): {oum:.4f} -> prob implícita ~ {fmt_prob_from_odd(oum)}")
    print(f" - Favorito da casa (menor odd do trio): {ofav:.4f} -> prob implícita ~ {fmt_prob_from_odd(ofav)}")

    if "mean_overround" in total:
        print("\nProbabilidades normalizadas (média) + overround:")
        print(f" - P(Home): {total['mean_prob_home_norm']*100:.2f}%")
        print(f" - P(Draw): {total['mean_prob_draw_norm']*100:.2f}%")
        print(f" - P(Away): {total['mean_prob_away_norm']*100:.2f}%")
        print(f" - Overround médio: {(total['mean_overround']*100):.2f}% (margem embutida)")

    if "mean_imp_prob_universe_raw" in total:
        print("\nProb implícita RAW (média) no universo (1/odd, sem normalizar):")
        print(f" - mean(1/odd) universo: {total['mean_imp_prob_universe_raw']*100:.2f}%")

    if stats.n_rows_with_result > 0:
