...
# Versão melhorada com validação cruzada, balanceamento e tuning automático
...
def main():
    ...
    MODEL_PATH = os.path.join(MODELS_DIR, "xgb_1x2_v2.json")
    META_PATH = os.path.join(MODELS_DIR, "xgb_1x2_v2_meta.json")
    COLS_PATH = os.path.join(MODELS_DIR, "xgb_1x2_v2_columns.pkl")
    SCALER_PATH = os.path.join(MODELS_DIR, "xgb_1x2_v2_scaler.pkl")
    ...
    # Garantias mínimas (inclui novas features)
    needed_cols = {
        "date", "target_1x2",
    ...
    # Remove outliers extremos (pode indicar dados ruins)
    ...
    # -------------------------
    # Features numéricas + novas features
    # -------------------------
    num_cols = [
        "elo_home_pre", "elo_away_pre", "elo_diff",
    ...
    # Adiciona novas features se existirem
    new_num_cols = [
    ...
    def make_X(d: pd.DataFrame) -> pd.DataFrame:
        Xn = d[num_cols].copy()
        # Preenche NaN em numéricas com 0
        Xn = Xn.fillna(0)
    ...
    # ✅ NORMALIZAÇÃO: StandardScaler nas numéricas (melhora generalizacao)
    ...
    # ✅ BALANCEAMENTO: class weights baseado na frequência
    ...
    print(f"\n[INFO] Iniciando tuning automático (max 50 trials)...")
    ...
    # Tuning com early stopping de trials
    ...
    # ✅ VALIDAÇÃO CRUZADA TEMPORAL para verificar generalização
    ...
    meta = {
        ...
        "improvements": [
            "Normalização StandardScaler em features numéricas",
            "Balanceamento automático de classes",
            "Tuning de hiperparâmetros com Optuna (50 trials)",
            "Validação cruzada temporal (5 folds)",
            "Remoção de outliers extremos",
            "6 novas features: form_avg, gd_avg, momentum, consistency",
    ...
    print("\n" + "-" * 70)
    print("[✅] Modelo v2 melhorado salvo em:", MODEL_PATH)
