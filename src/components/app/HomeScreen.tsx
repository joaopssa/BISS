...
export const HomeScreen: React.FC = () => {
  const [tab, setTab] = useState<"favoritos" | "em-alta" | "bilhetes">(
    "em-alta"
  );
  ...
  const fetchTickets = async () => {
    try {
      ...
      const mapped: Ticket[] = data.map((raw: any, idx: number) => {
        const id = String(
          raw.id_bilhete ?? raw.id ?? raw.bilheteId ?? `B${idx + 1}`
        );

        const createdAt =
          raw.data_criacao ?? raw.createdAt ?? raw.data_registro ?? new Date().toISOString();

        const stake =
          Number(
            raw.stake_total ?? raw.stake ?? raw.valor_apostado ?? 0
          ) || 0;

        const potentialReturn =
          Number(raw.possivel_retorno ?? raw.potentialReturn ?? 0) || 0;

        const status = raw.status ?? raw.status_bilhete ?? undefined;

        const selecoesRaw: any[] = Array.isArray(raw.selecoes)
          ? raw.selecoes
          : [];
  ...
  const handleOpenH2H = async (params: H2HRequest) => {
    ...
    try {
      // salva meta (times + logos) logo de cara
      setH2hMeta({
      ...
      if (!data.hasHistory) {
        ...
        setH2hOpen(true); // abre só para mostrar mensagem amigável
        return;
      }
  ...
  const filtered = useMemo(() => {
    ...
    // 1) Verifica se o texto digitado é um clube do clubs-map.json
    const matchedClub = Object.keys(clubsMap).find((club) =>
    ...
    // 2) Busca normal por time/competição
  ...
  const canAddSelection = (
    ...
    const sameMatch = current.filter(
      (s) => s.matchId === candidate.matchId || s.partida === candidate.partida
    );
    ...
    try {
      const payload = {
        ...
        apostas: selections.map((s) => ({
          ...
          time_casa: s.time_casa,
          time_fora: s.time_fora,
          data_hora_partida: new Date().toISOString(), // ajuste se tiver horário real
          mercado: s.mercado,
          selecao: s.selecao,
          odd: s.odd,
        })),
            ...
            {filteredTickets.length === 0 ? (
              ...
              <ul className="space-y-6">
                {filteredTickets.map((t) => {
                  const isPending =
                    !t.status || t.status.toUpperCase().trim() === "PENDENTE";
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Seleções — estilo luxo */}
                      <div className="space-y-4">
                        {t.selections?.map((s, i) => {
                          const logoHome = findClubLogo(
                            s.time_casa || s.match?.split(" x ")[0]
                          );
                          const logoAway = findClubLogo(
                            s.time_fora || s.match?.split(" x ")[1]
                          );

                          const leagueCode = s.campeonato
                            ? leagueCountries[s.campeonato]
                            : undefined;
  ...
  }, [tab, filtered, loading, error, tickets, userLeagues, filteredTickets]);
